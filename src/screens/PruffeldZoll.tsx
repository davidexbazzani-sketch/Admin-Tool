// ── Prüffeld & Zoll: Geräte auf dem Lageplan verorten ────────────────────────
// Wie OT-Geräte, nur mit dem Lageplan „Halle 1–11, Gesamtübersicht"
// (public/pruffeld-zoll/gesamtuebersicht.pdf) und EIGENER Geräteliste.
// Geräte per Klick platzieren, Hostname eintragen, in der Liste anklicken →
// Marker blinkt im Plan auf. Verlustfreies Zoomen via pdf.js (PlanViewer).

import { useCallback, useEffect, useMemo, useRef, useState } from 'react'
import { ClipboardCheck, Search, RefreshCw, Loader2, X, Trash2, MapPin, Crosshair, AlertTriangle, FileDown } from 'lucide-react'
...
} from '../services/pruffeldZoll'
import { exportPruffeldZollPdf } from '../services/pruffeldZollExport'

const PLAN_ASSET = 'pruffeld-zoll/gesamtuebersicht.pdf'

export default function PruffeldZoll() {
  ...
  async function exportPdf() {
    ...
    try {
      const r = await exportPruffeldZollPdf(devices)
  ...
  useEffect(() => {
    if (!intent || intent.screen !== 'pruffeld-zoll') return
  ...
  async function onPlace(x: number, y: number) {
    ...
    setPlacementMode(false)   // Hostname eintragen; für das nächste Gerät erneut aktivieren
  }
    ...
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="flex items-center gap-3 px-5 py-3 border-b border-border shrink-0">
        ...
        <div className="min-w-0">
          <h1 className="text-base font-semibold text-foreground leading-tight">Prüffeld & Zoll</h1>
          <p className="text-[11px] text-muted-foreground">{devices.length} Gerät(e) auf dem Lageplan verortet</p>
        </div>
        <div className="ml-auto flex items-center gap-2">
          ...
          >
            {placementMode ? <Crosshair size={14} /> : <MapPin size={14} />}{placementMode ? 'Platzieren aktiv – Plan anklicken' : 'Gerät platzieren'}
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {filtered.length === 0 ? (
              <div className="p-4 text-center text-xs text-muted-foreground">
                {devices.length === 0 ? 'Noch keine Geräte platziert. „Gerät platzieren" klicken und auf den Plan tippen.' : 'Keine Treffer.'}
              </div>
            ) : filtered.map(d => {
        ...
        <main className="flex-1 min-w-0">
          <PlanViewer
            ...
            onPageCountKnown={() => { /* Gesamtübersicht kann mehrere Seiten haben */ }}
        ...
        {selected && (
          <aside className="w-72 shrink-0 border-l border-border flex flex-col">
            <div className="flex items-center gap-2 px-3 py-2.5 border-b border-border">
              ...
              <span className="text-sm font-semibold text-foreground">Geräte-Details</span>
            ...
            <div className="p-3 space-y-3 overflow-y-auto">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="z. B. Prüffeld / Platz 3"
              ...
              <button onClick={removeSelected} className="w-full inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-md border border-red-500/40 text-red-300 hover:bg-red-500/10 text-sm">
                <Trash2 size={14} />Gerät entfernen
              </button>
            </div>
          </aside>
        )}
