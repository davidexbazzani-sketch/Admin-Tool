// ── OT-Geräte – PDF-Export ───────────────────────────────────────────────────
// Erzeugt ein PDF:
//   Seite 1 = der Hallenplan (Original-Vektorseite via pdf-lib) mit den PC-Markern
//             (Punkt + Hostname direkt am Standort)
//   ab Seite 2 = Liste aller PCs mit Hostname + Arbeitsplatz (durchnummeriert,
//                passend zu den Marker-Nummern auf dem Plan)
...
export async function exportOtDevicesPdf(devices: OtDevice[]): Promise<{ ok: boolean; cancelled?: boolean; error?: string }> {
  const savePath = await api().saveFileDialog(
    `OT-Geraete Hallenplan ${new Date().toISOString().slice(0, 10)}.pdf`,
  ...
  try {
    ...
    out.setTitle('OT-Geräte – Hallenplan')
    ...
    // ── Seite 1+: Hallenplan mit Markern ──
    ...
    if (asset.success && asset.data) {
      try {
        ...
        page.drawText('Hallenplan konnte nicht eingebunden werden.', { x: 36, y: 800, size: 12, font: fontBold, color: COLOR_TEXT })
        page.drawText(String(e), { x: 36, y: 780, size: 9, font, color: COLOR_MUTED })
      }
    } else {
      const page = out.addPage([595, 842])
      page.drawText('Hallenplan nicht gefunden.', { x: 36, y: 800, size: 12, font: fontBold, color: COLOR_TEXT })
    }

    // ── Liste: alle PCs mit Arbeitsplatz ──
    ...
    const drawHeader = (cont: boolean) => {
      page.drawText(`OT-Geräte – Arbeitsplätze${cont ? ' (Forts.)' : ''}`, { x: M, y: y - 16, size: cont ? 13 : 18, font: fontBold, color: COLOR_HEAD })
      if (!cont) page.drawText(`${numbered.length} PC(s) · Stand ${fmtDateTime(new Date())}`, { x: M, y: y - 32, size: 9, font, color: COLOR_MUTED })
      y -= cont ? 30 : 46
      // Spaltenköpfe
