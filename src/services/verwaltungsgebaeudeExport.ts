// ── Verwaltungsgebäude – PDF-Export ──────────────────────────────────────────
// Wie otDevicesExport.ts, nur mit dem Lageplan „KG-Staffelgeschoss Übersicht":
//   Seite 1 = Lageplan (Original-Vektorseite via pdf-lib) mit den Geräte-Markern
//   ab Seite 2 = Liste aller Geräte mit Hostname + Raumnummer (durchnummeriert)
...
const COLOR_LOCK = rgb(0.85, 0.12, 0.12)
...
/** Marker „verschlossene Tür" (rotes Quadrat + optionale Bezeichnung) zeichnen. */
function drawLock(page: PDFPage, font: PDFFont, x: number, y: number, label: string) {
  const s = 5.5
  page.drawRectangle({ x: x - s / 2 - 1, y: y - s / 2 - 1, width: s + 2, height: s + 2, color: COLOR_MARKER_BORDER, opacity: 0.9 })
  page.drawRectangle({ x: x - s / 2, y: y - s / 2, width: s, height: s, color: COLOR_LOCK })
  const clean = label || 'verschlossen'
  const fontSize = 6
  const padX = 2, padY = 1.4
  const tw = font.widthOfTextAtSize(clean, fontSize)
  const th = font.heightAtSize(fontSize)
  const lx = x + s / 2 + 3
  const ly = y - th / 2 + 1
  page.drawRectangle({ x: lx - padX, y: ly - padY, width: tw + 2 * padX, height: th + 2 * padY, color: COLOR_LOCK, opacity: 0.85 })
  page.drawText(clean, { x: lx, y: ly, size: fontSize, font, color: COLOR_LABEL_FG })
}

/** Natürliche Sortierung nach Raumnummer (Zahlen numerisch), dann Hostname. */
...
export async function exportVerwaltungsgebaeudePdf(devices: OtDevice[]): Promise<{ ok: boolean; cancelled?: boolean; error?: string }> {
  const savePath = await api().saveFileDialog(
    `Verwaltungsgebaeude Lageplan ${new Date().toISOString().slice(0, 10)}.pdf`,
  ...
  try {
    ...
    out.setTitle('Verwaltungsgebäude – Lageplan')
    out.setProducer('IT Admin Tool'); out.setCreator('IT Admin Tool'); out.setCreationDate(new Date())
    const font = await out.embedFont(StandardFonts.Helvetica)
    const fontBold = await out.embedFont(StandardFonts.HelveticaBold)

    // Geräte werden nummeriert + gelistet; verschlossene Türen nur auf dem Plan.
    const deviceList = devices.filter(d => d.kind !== 'lock')
    const lockList = devices.filter(d => d.kind === 'lock')
    const sorted = sortDevices(deviceList)
    const numbered = sorted.map((d, i) => ({ d, n: i + 1 }))
    const numByPage = (page: number) => numbered.filter(x => (x.d.page || 1) === page)
    const locksByPage = (page: number) => lockList.filter(d => (d.page || 1) === page)
    ...
    if (asset.success && asset.data) {
      try {
        ...
        copied.forEach((page, pageIdx) => {
          ...
          for (const d of locksByPage(pageIdx + 1)) {
            drawLock(added, fontBold, d.x * pw, (1 - d.y) * ph, d.hostname || '')
          }
        })
      } catch (e) {
    ...
    // ── Liste: alle Geräte mit Raumnummer ──
    ...
    const drawHeader = (cont: boolean) => {
      page.drawText(`Verwaltungsgebäude – Räume${cont ? ' (Forts.)' : ''}`, { x: M, y: y - 16, size: cont ? 13 : 18, font: fontBold, color: COLOR_HEAD })
      if (!cont) page.drawText(`${numbered.length} Gerät(e)${lockList.length ? ` · ${lockList.length} verschl. Tür(en)` : ''} · Stand ${fmtDateTime(new Date())}`, { x: M, y: y - 32, size: 9, font, color: COLOR_MUTED })
      y -= cont ? 30 : 46
      // Spaltenköpfe
      ...
      page.drawText('Raumnummer', { x: cPlace, y, size: 8.5, font: fontBold, color: COLOR_MUTED })
